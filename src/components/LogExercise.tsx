...
const categoryColors: Record<ExerciseCategory, string> = {
  ...
  Core: 'bg-green-900/30 text-green-500 border-green-700/50',
...
export function LogExercise({ userEmail }: LogExerciseProps) {
  ...
  const [selectedExercise, setSelectedExercise] = useState<{ name: string; category: ExerciseCategory } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addLog(selectedExercise.name, selectedExercise.category, parseFloat(weight), undefined, undefined, undefined, selectedDate);
      ...
      <div className="h-full flex flex-col bg-black">
        <div className="bg-gray-950/80 backdrop-blur-xl border-b border-green-900/30 p-4">
          ...
          <div className="flex items-center gap-2">
            <div className={`flex-1 h-1 rounded-full transition-all ${formStep >= 1 ? 'bg-green-600' : 'bg-gray-900'}`}></div>
            <div className={`flex-1 h-1 rounded-full transition-all ${formStep >= 2 ? 'bg-green-600' : 'bg-gray-900'}`}></div>
          </div>
        </div>
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {filteredExercises.map((exercise) => (
                  <button
                    key={exercise.name}
                    onClick={() => handleExerciseSelect(exercise.name, exercise.category)}
                  ...
                  >
                    ...
                    <h4 className="text-green-400 font-semibold text-sm mb-1 line-clamp-2">{exercise.name}</h4>
                    <span className={`inline-block text-xs px-2 py-0.5 rounded-md border ${categoryColors[exercise.category]}`}>
                      {exercise.category}
                    </span>
                  </button>
                ))}
            ...
            <form onSubmit={handleSubmit} className="max-w-md mx-auto">
              <div className="bg-gray-950/80 backdrop-blur-xl rounded-2xl p-6 border border-green-900/30 mb-6">
                <div className="text-center mb-6">
                  <h3 className="text-2xl font-bold text-green-400 mb-2">{selectedExercise?.name}</h3>
                  <span className={`inline-block text-xs px-3 py-1 rounded-md border ${categoryColors[selectedExercise?.category || 'Back']}`}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-green-500 mb-2">End Weight (kg)</label>
                    <input
                      type="number"
                      step="0.5"
                      value={weight}
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {loading ? (
          ...
          <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center gap-2">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                              <h4 className="text-green-400 font-semibold text-sm truncate">{log.exerciseName}</h4>
                              <span className={`inline-block text-xs px-1.5 py-0.5 rounded border ${categoryColors[log.category]} flex-shrink-0`}>
                                {log.category}
                              </span>
                            </div>
                            <span className="text-base font-bold text-green-500 flex-shrink-0">{log.weight} kg</span>
                          </div>
                          <button
            ...
            {recentLogs.length === 0 && (
              <div className="text-center py-8">
                <p className="text-green-700 text-sm">No logs in the last 14 days. Start tracking your progress!</p>
              </div>
            )}
          </div>
        )}
